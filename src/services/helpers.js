...
export const dateToLongMsg = (date) => {
  const parsedDate = new Date(date);
  return (
    `[${format(parsedDate, 'EEEEEE').toUpperCase()}] ` +
...
export const checkAllDaysPicked = (keyboard) => {
  for (let i = 0; i < keyboard.length - 1; i++) {
    for (let j = 0; j < keyboard[i].length; j++) {
      if (
        !(
          keyboard[i][j].text.startsWith(Constants.DAY_SYMBOL) ||
...
export const generateList = (keyboard, preferences) => {
  ...
  for (let i = 0; i < keyboard.length - 1; i++) {
    for (let j = 0; j < keyboard[i].length; j++) {
      ...
      const [shiftStart, shiftEnd] = firstShift
...
export const genShiftPair = (shift) => {
  ...
  if (shift.timeEnd.nextDay) {
    date = format(
      addDays(parse(date, Formats.dateMonth, new Date()), 1),
      Formats.dateMonth,
    ).toUpperCase();
  }
...
export const createEventsList = (eventsArray, eventName) => {
  eventsArray = eventsArray.filter((event) => event.summary === eventName);
  ...
  eventsArray = eventsArray.sort(
    (event1, event2) => event1.data.start.dateTime - event2.data.start.dateTime,
  );
  ...
  if (currentDate < eventsArray[0].data.start.dateTime) {
    ...
    currentDate > eventsArray[eventsArray.length - 1].data.start.dateTime
  ) {
